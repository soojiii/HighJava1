...
public class FileUpload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//GET방식으로의 요청이 오면 Upload Form 화면이 출력되도록 한다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Part part : request.getParts()) {
			...
			if(fileName!=null) {  //파일인지 검사
				...
				try {
					//Upload된 파일을 저장한 '업로드 폴더'에 저장하기
					//저장하는 메서드 ==> Part객체.weite()메서드 이용
	 ...
	 * 2) 파일일 경우
	 *  * ---------------231djksls ==> Part들을 구분한 구분선
	 * content-disposition : form-data; name="upFile1"; filename="test1.txt"==>파일경로
